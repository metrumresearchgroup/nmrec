...
`?nmrec_option` objects, like `?nmrec_record` objects, are R6 objects.
The fields of interest depend on the option type.  Here are the
important fields for the selected method option of the first
estimation record:
...
If you print `ctl` (the original `nmrec_ctl_records`), you'll notice
that all of the changes above are reflected there.  You can write the
changes to a file with `write_ctl()`.
